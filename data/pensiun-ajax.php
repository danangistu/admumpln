<?php
	...
	try{
		$rows=R::getAll("	SELECT p.id, p.nip, p.nama, p.jabatan, o.org_unit, p.birthdate,FLOOR(DATEDIFF(CURDATE(),birthdate)/365) as umur ,DATE_ADD(p.birthdate, INTERVAL 56 YEAR) as tgl_pensiun
							FROM pegawai p, orgunit o
							WHERE p.id_org_unit=o.id_org_unit AND (FLOOR(DATEDIFF(CURDATE(),birthdate)/365)='54' OR FLOOR(DATEDIFF(CURDATE(),birthdate)/365)='55' OR FLOOR(DATEDIFF(CURDATE(),birthdate)/365)='56')
